// Package cli builds the pixiv command tree on top of the pixiv library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: pixiv.DefaultConfig()}

	root := &cobra.Command{
		Use:   "pixiv",
		Short: "A command line for Pixiv.",
		Long: `pixiv reads public Pixiv ranking data from pixiv.net.
No account or API key required. It returns records as table, JSON, JSONL,
CSV, TSV, or URLs.

pixiv is an independent tool and is not affiliated with Pixiv Inc.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = pixiv.NewClient(a.cfg)
	return nil
}
